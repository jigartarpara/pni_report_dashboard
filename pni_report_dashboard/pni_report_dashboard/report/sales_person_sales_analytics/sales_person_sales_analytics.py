...
class Analytics(object):
	...
	def get_sales_transactions_based_on_item_group(self):
		...
		self.entries = frappe.db.sql("""
			...
			from `tab{doctype} Item` i , `tab{doctype}` s, `tabSales Team` st
			where s.name = i.parent and i.docstatus = 1 and s.company = %s
			and st.parent = s.name and st.sales_person = '{sales_person}'
			and s.{date_field} between %s and %s
		""".format(date_field=self.date_field, value_field=value_field, doctype=self.filters.doc_type, sales_person = self.get_current_sales_person() ),
		(self.filters.company, self.filters.from_date, self.filters.to_date), as_dict=1)
